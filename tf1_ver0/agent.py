...
class Agent:
    def __init__(self, args):
        ...
        self.n_past_steps = args.n_past_steps
        ...
        with tf.variable_scope(self.name):
            ...
            kl_old = tf.reduce_mean(tf.reduce_sum(self.old_dist.kl_divergence(self.dist), axis=1))
            kl_mu = tf.reduce_mean(tf.reduce_sum(self.mu_dist.kl_divergence(self.old_dist), axis=1))
            kl_mu = tf.sqrt(kl_mu*(self.max_kl + 0.25*kl_mu)) - 0.5*kl_mu
            self.kl = kl_old + kl_mu
            ...
            prob_ratios = tf.exp(log_probs - mu_log_probs)
            old_prob_ratios = tf.exp(old_log_probs - mu_log_probs)
            if self.clip_value > 0:
                prob_ratios = tf.clip_by_value(prob_ratios, 1.0 - self.clip_value, 1.0 + self.clip_value)
                old_prob_ratios = tf.clip_by_value(old_prob_ratios, 1.0 - self.clip_value, 1.0 + self.clip_value)
            gaes_mean = tf.reduce_mean(self.gaes_ph*old_prob_ratios)/(tf.reduce_mean(old_prob_ratios) + EPS)
            gaes_std = tf.math.reduce_std(self.gaes_ph)
            self.objective = tf.reduce_mean(prob_ratios*(self.gaes_ph - gaes_mean)/(gaes_std + EPS))
            self.grad_g = tf.gradients(self.objective, p_vars)
            self.grad_g = tf.concat([tf.reshape(g, [-1]) for g in self.grad_g], axis=0)

            #cost value at risk
            cost_gaes_mean = tf.reduce_mean(self.cost_gaes_ph*old_prob_ratios)/(tf.reduce_mean(old_prob_ratios) + EPS)
            cost_var_gaes_mean = tf.reduce_mean(self.cost_var_gaes_ph*old_prob_ratios)/(tf.reduce_mean(old_prob_ratios) + EPS)
            self.approx_cost_mean = self.cost_mean_ph + (1.0/(1.0 - self.discount_factor))*(tf.reduce_mean(prob_ratios*(self.cost_gaes_ph - cost_gaes_mean)))
            self.approx_cost_var = self.cost_var_mean_ph + (1.0/(1.0 - self.discount_factor**2))*(tf.reduce_mean(prob_ratios*(self.cost_var_gaes_ph - cost_var_gaes_mean)))
    ...
    def train(self, trajs):
        ...
        for env_idx in range(self.n_envs):
            ...
            n_past_steps = min(len(self.replay_buffer[env_idx]), self.n_past_steps_per_env)
            start_idx = len(self.replay_buffer[env_idx]) - n_past_steps
            ...
            if len(self.replay_buffer[env_idx]) >= self.n_update_steps_per_env and self.n_update_steps_per_env > n_past_steps:
                start_idx = np.random.randint(0, len(self.replay_buffer[env_idx]) - self.n_update_steps_per_env + 1)
                end_idx = start_idx + (self.n_update_steps_per_env - n_past_steps)
                env_trajs = list(self.replay_buffer[env_idx])[start_idx:end_idx]

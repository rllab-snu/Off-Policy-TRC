...
def draw(env_name, item_list, algo_list, fig_size, window_size, interp_steps, is_horizon=True):
    ...
    for item_idx in range(len(item_list)):
        ...
        for algo_idx in range(len(algo_list)):
            ...
            print(f"env_name: {env_name}, item_name: {item_name}, algo_name: {algo_name}", end=' || ')
            ...
            ax.fill_between(linspace, means - stds, means + stds, alpha=0.2)
...
def parse(algo_dirs, item_name, window_size, interp_steps):
    ...
    # ======= for logging ======= #
    num_steps = 1000
    num_steps_per_algos = int(num_steps/len(algo_datas))
    concat_data = np.concatenate([algo_data[-num_steps_per_algos:] for algo_data in algo_datas])
    mean = np.mean(concat_data)
    std = np.std(concat_data)
